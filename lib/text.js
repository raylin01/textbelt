...
const apppwd = 'nvvbhmhmajrjgovt';
const emailaddress = 'cumtdreminder@gmail.com';
...
module.exports.sendText = function (phone, message, opts, cb) {
  ...
  var provider = opts.provider ? opts.provider : default_options.provider;

  var providers_list = providers[region];

  var usproviders_list = require('./usproviders.json');
  ...
  const send = require('gmail-send')({
    user: emailaddress,
    pass: apppwd,
    to:   usproviders_list[provider].replace('%s', phone),
    subject: "CUMTD Bus App",
    text: message,
  });
  const result = send().then(function(){
  });
